...
1. ドメイン コントローラー (**LON-DC1**) のタスクバーで 「**Server Manager**」 アイコンを選択します (すでに開いている場合)。開いていない場合は開いてください。

2. 「**Server Manager**」 の右上にあるメニューバーで 「**Tools**」 を選択し、ドロップダウン メニューで 「**Group Policy Management**」 を選択します。

3. 必要に応じて 「**Group Policy Management**」 ウィンドウを最大化します。
...
5. 「**Group Policy Management Editor**」 ウィンドウを最大化します。

6. 右側のペインの 「**Default Domain Policy**」 ツリーで 「**Computer Configuration**」  >  「**Policies**」  >  「**Windows Settings**」  >  「**Security Settings**」  >  「**Account Policies**」 を参照します。

7. 「**Account Policies**」 フォルダーで 「**Account Lockout Policy**」 を選択します。
...
★しきい値どおりに動作しない場合があります。その場合はそのまま手順をお進みください★
