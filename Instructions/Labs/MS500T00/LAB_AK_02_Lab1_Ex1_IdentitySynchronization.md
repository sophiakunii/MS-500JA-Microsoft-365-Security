...
5.	「**AD DS に接続**」 画面でドメイン管理者 **ADATUM\Administrator** とパスワード `Pa55w.rd` を入力して、「**次へ**」 を選択します。   
6.	「**Continue without matching all UPN suffixes to verified domains**」 チェックボックスを選択します。Azure AD サインイン構成画面で 「**次へ**」 を選択します。   
7.	「**構成の準備完了**」 画面で 「**構成が完了したら、同期プロセスを開始**」 チェックボックスが選択されていることを確認してから 「**インストール**」 を選択します。   
8.	インストールが完了するのを待ちます (数分かかる可能性があります)。   
9.	「**終了**」 を選択します。   
